...
RolloutCore measured the shape of that problem. In `results/phase5.json`
(`docs/phase5-results.md`), `R-long` was admitted while the engine reported `rc-0`,
and the update to `rc-1` was requested while it was still generating. RolloutCore's
drain held the mutation until the request had returned, so the engine's label **at
the request's completion** was still `rc-0`, while its label **after the cycle** is
`rc-1`. Nothing in the engine distinguishes those two moments for a given request,
and nothing binds either to the response:

| request | bound version | label at admission | label at completion | label after the cycle |
|---|---|---|---|---|
| `R-long` | `rc-0` | `rc-0` | `rc-0` | **`rc-1`** |
| `R2` | `rc-1` | `rc-1` | `rc-1` | `rc-1` |
...
Phase 4A run of the same shape reproduced the pre-update 16-token baseline prefix
exactly (`results/phase4a.json`). (The committed Phase 5 artifact records `rc-1`
in its `engine_version_at_completion` field; that value was read after the cycle
rather than at completion. The defect and its fix are documented in
`docs/phase5-results.md`; the corrected reading is the table above.)

Two things follow for the open contract question:

1. **A post-hoc read of `weight_version` names the version current when you asked,
   not the one that produced the tokens.** A client that wants attribution has to
   record it at admission, which is what `engine_version_at_admission` and the
   binding do.
2. **The reason #49040 removed per-request binding is real**: for `mode="keep"`, a
   request spans versions. RolloutCore's drain is what makes that unreachable, and
   the price is holding the update until the longest in-flight generation
   finishes: 1.375 s in Phase 4A, unbounded in principle (Phase 4B's failed-drain
   case). Attribution can be the caller's job, but only if the caller also owns
   the drain.

This is a data point for the open question, not a proposal to change the API.
...
semantics, and (importantly) that `clear_cache` changes prefix-cache behaviour:
`test_clear_cache_preserves_output_and_controls_prefix_cache`
...
> Exit criterion 7, **Post-update cache coherence**: "#48762 or an equivalent
> non-reverted fix lands; model, adapter, target, and draft updates either
> invalidate every affected cache or advance the generation used in cache
> identity."
...
6. Reuse the *same* request. Assert (a) `cached_tokens` is `0`, so the cache path
...
proposed design prefers the engine-side completion gate, which may be what a
previous worker-side patch failed to satisfy.
...
relies on this, stamping the version label as the salt
...
controller by design, because a frozen request can span two weight versions. The
defect is real for anyone reading that docstring.
...
`vllm/distributed/device_communicators/pynccl.py:95-100`), while the file's only
timeout is on teardown, and its own comment records that a failed join
...
**Verdict: report, as a design discussion rather than a patch.** In V1 the version label
...
the holder disappears, an RFC-scale question appropriate for the #48312 / #48306
track rather than a drive-by PR. The contribution is the measured failure:
...
| 2 | **PR** (the one to open first) | "Bound the RL weight-update collective, and document pause/cache honestly": a timeout at the three RL call sites plus route plumbing (§3.4), the `/pause` `clear_cache` docstring correction (§3.3), and the missing invalidation step in `docs/training/async_rl.md` and `docs/training/weight_transfer/README.md`. One PR, one story, same subsystem | `results/phase6-8b-with-deepkill.json` (`hung: true`, 120 s), `results/phase4d.json` (0.177 s at 125M), the `/collective_rpc` precedent at `vllm/entrypoints/serve/dev/rpc/api_router.py:42`, the three pause paths, and `docs/training/async_rl.md` using `mode="keep", clear_cache=True` in its own example |
| 3 | **Issue** | "Which commit point should invalidate caches after an update?": the §3.1 question, referencing #48762 and RFC exit criterion 7, with §2's lane as the way to decide | `vllm/v1/worker/gpu_worker.py:1488-1505`, `vllm/v1/engine/core.py:841-846` (the code's own statement of intent), `results/phase4c.json` (`kv_reused_across_weights: true`) |
| 4 | **Issue** | "Reserve headroom for weight-transfer receive buffers in the KV budget": §3.6 | `vllm/v1/worker/gpu_worker.py:661-665`, `vllm/distributed/weight_transfer/packed_tensor.py:253`, the measured OOM text and the flags that avoided it (`results/phase6-8b.json`) |
| 5 | **Comment on RFC #48312** | offer the §2 Lane A implementation and the §3.5 lease finding for the tracker | `results/phase4b.json` (`AlreadyManagedEngineError` text), §3.5 |
| 6 | **Comment on RFC #48306 §2.2** | the §1.1 four-column table, as evidence for the open contract question | `results/phase5.json` (bound and admitted at `rc-0`; the corrected completion reading in `docs/phase5-results.md`), `results/phase4a.json` (identical prefix to the pre-update baseline) |

Not proposed: a patch for the encoder/MM invalidation (§3.2), already tracked
upstream, and a prior attempt (#48762) was closed, so the question in row 3 comes
first. Also not proposed: the `/pause` docstring as a standalone change (it is
row 2's tail, not a PR of its own; see §5.1), and any change to per-request
version binding; §1.1 is evidence for a discussion, and the removal by #49040 was
deliberate.

## 5. Would a PR actually be accepted? A grounded answer

This section exists because "should we open a PR?" deserves an answer grounded in
what vLLM asks of contributors, not in enthusiasm. All of it is from the pinned
checkout.

### 5.1 What upstream actually requires

From `docs/contributing/README.md` and `.github/PULL_REQUEST_TEMPLATE.md`:

| Requirement | Detail |
|---|---|
| **DCO** | every commit needs a `Signed-off-by:` header (`docs/contributing/README.md:182-186`; `DCO`) |
| **Tests** | `pytest tests/` is the documented runner; the repo is not fully `mypy`-checked and CPU test coverage is partial (`docs/contributing/README.md:150-166`) |
| **PR template** | Purpose / Test Plan / Test Result, with the test command and before/after evidence (`PULL_REQUEST_TEMPLATE.md`) |
| **Title** | bracketed type tag first, e.g. `[Bugfix][RL] …` (`docs/contributing/README.md:222-233`) |
| **AI contributions** | *"Do not submit 'pure agent' PRs"*; the human submitter reviews every changed line, validates end-to-end and runs the tests; *"Avoid one-off 'busywork' PRs (single typo, isolated style cleanup…). Bundle mechanical cleanups into a clear, systematic scope"*; disclose AI assistance in the PR body and use `Co-authored-by:`-style trailers (`docs/contributing/README.md:194-218`) |

Two consequences follow immediately, and they change the plan in §4:

1. **The `/pause` docstring fix should not be filed on its own.** It is exactly the
   "single typo" class the AI policy names. It has to travel with something
   substantive in the same subsystem.
2. **Nothing here can be a fire-and-forget agent PR.** Every line has to be
   defensible by the human who submits it, and the evidence in this repo is what
   makes that possible.

### 5.2 The three candidates, ranked by value ÷ risk

| Candidate | What ships | Acceptance risk | Verdict |
|---|---|---|---|
| **1. The RL update path can hang, and its cache semantics are documented inconsistently.** One PR: pass a timeout at the three `AsyncLLM` call sites + route plumbing, correct the `/pause` `clear_cache` docstring, and add the missing invalidation step to the training docs | ~40 lines of code, ~20 lines of docs, one or two tests | **Low.** The timeout parameter exists end to end (`async_llm.py:1142`, `core.py:1033`, `core_client.py:1329`) and the sibling `/collective_rpc` route already exposes it (`dev/rpc/api_router.py:42`); the docstring contradicts the code in all three pause paths and contradicts `docs/training/async_rl.md`, which uses `mode="keep", clear_cache=True` in its own example | **Best first PR.** It is a bugfix with a measured symptom (Phase 6: a 120 s hang), it is mechanical, and it has a natural test plan |
| **2. An end-to-end cache-coherence regression test** (the §2 proposal) | a new test file under `state_transitions/`, plus a synthetic trainer or the mock-engine trick | **Medium.** It *is* the RFC exit criterion, but it needs CI fixtures that do not exist yet, and the lane's design is unsettled while #48312 is open | **Issue first, with an offer to implement Lane A.** A PR here without maintainer buy-in risks duplicating in-flight work |
| **3. Reserve KV headroom for transfer receive buffers** | a documented/configurable reservation | **Medium-low**, but it is a policy question (who owns the last gigabyte) | **Issue**, carrying the measured OOM |

### 5.3 What not to do

* **Do not file the docstring fix alone.** Bundled, it is a real inconsistency; alone
  it is busywork by upstream's own written policy.
* **Do not propose the encoder/MM invalidation patch** (§3.2): already tracked, and
  a previous attempt (#48762) was closed. The question comes first (§3.1).
* **Do not make anything depend on RolloutCore.** A PR is a patch to vLLM; our
  runtime can supply the *evidence*, never a required dependency.
* **Do not describe the work as agent-generated without saying so.** The policy is
  explicit, and the disclosure is cheap.
* **Do not expect a fast merge.** #48312 is open with many per-category PRs in
  flight; a coherent bugfix is the item that does not need the design settled.

### 5.4 The mechanics, if we do it

```bash
git clone https://github.com/vllm-project/vllm && cd vllm
git checkout -b rl-update-timeout-and-cache-docs main
# ... the change ...
pytest tests/entrypoints/serve/dev/rlhf/ -x -q     # the lane this touches
git commit -s    # DCO: Signed-off-by is required, not optional
```

The PR body should carry: the measured symptom (`results/phase6-8b-with-deepkill.json`,
`hung: true`, 120 s), the sibling-route precedent, the doc contradiction with the
training guide's own example, the exact test commands run, the environment
(`detail["environment"]`, `docs/phase6-results.md`), and a disclosure line for AI
assistance. Mark it ready, not draft, if the only open question is the timeout
*default*; mark it draft if that value needs a maintainer's decision.

### 5.5 The honest bottom line

**A PR to `main` works, and the small bugfix is the one worth opening.** The
median outcome for a drive-by docstring fix in a project this busy is a stalled
PR; the median outcome for a *measured hang with a small diff and a test* is a
review. The coherence test is the more valuable contribution and the riskier
first move, so it should start as an issue with our two-lane plan attached.

None of this is a promise of acceptance. It is a claim about which of our
findings is legible to a maintainer in under five minutes, and that is the timeout
plus the documentation inconsistency around it.
