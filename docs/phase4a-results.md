...
**The guarantee is about *delay*, not about survival.** RolloutCore does not let
a generation run across a mutation and hope it comes out intact. It refuses to
mutate while the generation is alive:
...
`chars_matching_baseline_prefix: 48` out of `baseline_prefix_chars: 48`: the whole
16-token baseline. A 256-token generation whose prefix reproduces the pre-update
baseline, token for token, is one that ran on the old weights throughout.
`ignore_eos` forces the 256 tokens exactly, so the request could not finish early
and quietly stop overlapping the cycle.
...
(`lifecycle.py:465-472`) because the engine claimed quiescence while RolloutCore
still counted live work.
...
when the *cycle* began: the precondition for the experiment, not a claim about the
mutation. The harness now names it `inflight_overlapped_the_cycle`; as with
