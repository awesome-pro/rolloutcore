...
The headline: **130.63× cheaper than a restart at 125M, 8.71× at 8B** — and the
gap narrows with model size because the restart's cost is mostly fixed engine
startup while the hot path's cost is the broadcast itself.
...
| | |
|---|---|
| Date | 2026-09-24 18:06–18:25 UTC |
...
**deliberately generous to the baseline**: it assumes the new checkpoint is
already on disk. A real restart-based update must also *write* a checkpoint first,
which is not counted, so the gap below is a floor on the hot path's advantage.
...
poll interval and nothing more. What the hot path costs is exactly what moves over
the wire — 0.1008 s for a ~250 MB checkpoint, 4.1287 s for a ~16 GB one.
...
token costs ~0.3–0.5 s more than `/health`, and both runs serve the same text the
hot path produced, which is the evidence that the baseline really installed the
target weights.
