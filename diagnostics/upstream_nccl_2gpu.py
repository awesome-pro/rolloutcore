...
2-GPU layout (single node):
  Inference — GPU 0, `vllm serve` with TP=1
  Training  — GPU 1, a bf16 Hugging Face model in this process

ADAPTED FROM UPSTREAM for Phase 3B of RolloutCore. The upstream file is
3-GPU (TP=2 inference + fp8 + trainer on GPU 2); this copy is reduced to the
two GPUs a Phase 3B pod has, with fp8 dropped so the only variable under test is
NCCL. See docs/phase3b-runbook.md section 4 for the diff and the reasoning.
...
def start_vllm_server() -> subprocess.Popen:
    ...
    # mode="wait" (below) is refused by an in-process engine
    # (vllm/v1/engine/core.py:902), so make the out-of-process core explicit.
...
def pause_generation(base_url: str) -> None:
    ...
    ``mode="wait"`` lets an in-flight request finish instead of aborting it,
    which is the drain semantics RolloutCore requires (plain /pause defaults to
    mode="abort").
    """
    requests.post(f"{base_url}/pause?mode=wait", timeout=600).raise_for_status()
...
def print_generations(label: str, prompts: list[str], outputs: list[str]) -> None:
    ...
    print(label)
    print("-" * 50)
    for prompt, generated_text in zip(prompts, outputs, strict=True):
