...
`docs/`: `docs/phase3c-results.md`, `phase4a` through `phase4d`, `phase5`,
`phase6`. What this directory is *not*, then, is a set of throwaway probes. It is
...
| `rolloutcore_concurrency_2gpu.py` | 4A | an update requested while a 256-token generation is in flight is **held off** until it finishes, so no rollout spans a mutation |
...
`rolloutcore_benchmark_2gpu.py` now records the whole environment before it starts
the server. `detail["environment"]` holds GPU names and memory, driver, compute
capability, torch, CUDA, NCCL, per-pair `p2p_peer_access`, and the raw
`nvidia-smi topo -m`. The earlier artifacts predate that block; they are left as
...
its 3-GPU layout to a 2-GPU pod. The four em dashes in that file are upstream's
own comments, kept because the file is a vendored copy; nothing else in the
repository has one.
...
| (none) | `VLLM_ENABLE_V1_MULTIPROCESSING=1` | `mode="wait"` is refused by an in-process engine (`vllm/v1/engine/core.py:902`) |
...
the failure surfaces on the HTTP socket (0.177 s, `ConnectionResetError(104)`), so
